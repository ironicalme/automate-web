...
class TestConfigWithAWSFallback:
    ...
    def test_get_config_with_aws_fallback_aws_success(self):
        ...
        with (
            patch(
                "automate_ui.common.utils.aws_secrets.check_aws_credentials",
                return_value=True,
            ),
            patch(
                "automate_ui.common.utils.aws_secrets.get_secret_from_aws",
    ...
    def test_get_config_with_aws_fallback_yaml_fallback(self):
        ...
        try:
            with (
                ...
                assert (
                    result["api"]["public"]["base_url"]
                    == "https://yaml-api.example.com"
                )
        finally:
            os.unlink(temp_file)
    ...
    def test_get_config_with_aws_fallback_both_fail(self):
        ...
        with (
            ...
            assert (
                "Configuration not available from AWS Secrets Manager or YAML file"
...
class TestNestedConfigValue:
    ...
    def test_get_nested_config_value_aws_success(self):
        ...
        aws_config = {"api": {"public": {"key": "aws-api-key"}}}

        with patch(
            "automate_ui.common.utils.aws_secrets.get_config_with_aws_fallback",
            return_value=aws_config,
        ):
            result = get_nested_config_value(
                "local_secrets.yaml", "api.public.key", "test-secret"
            )
        ...
        try:
            with patch(
                ...
                mock_get_config.return_value = {
                    "api": {"public": {"key": "yaml-api-key"}}
                }

                result = get_nested_config_value(
                    temp_file, "api.public.key", "test-secret"
                )
        ...
        with patch(
            ...
            with pytest.raises(KeyError) as exc_info:
                get_nested_config_value(
                    "local_secrets.yaml", "api.public.nonexistent", "test-secret"
                )
