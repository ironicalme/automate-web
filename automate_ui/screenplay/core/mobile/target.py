...
class Target:
    ...
    def _wait_for(self, driver, locator_kwargs, timeout: float, multiple: bool) -> Union[WebElement, list[WebElement]]:
        ...
        return wait.until(lambda x: x.find_elements(by=by, value=locator_kwargs["value"]) if multiple else x.find_element(by=by, value=locator_kwargs["value"]))

    def _find_element_with_fallback(self, actor: Actor, timeout: float, multiple: bool) -> Union[WebElement, list[WebElement]]:
        """
        ...
        if driver is None:
            raise TargetingError(f"There is no active mobile session! {actor} cannot find the {self}.")

        if not self.locator_strategy:
            self._set_locator_from_fallback(platform)

        if not self.locator_strategy:
            raise TargetingError(f"There is no primary or fallback locator defined for {self.target_name} on {platform}")

        locator_strategy_name = self.locator_strategy.name
        if not hasattr(AppiumBy, locator_strategy_name):
            raise TargetingError(f"AppiumBy locator method: '{locator_strategy_name}' not found/implemented")
    ...
    def found_by(self, actor: Actor, timeout: float = Timeouts.APPIUM_WAIT) -> WebElement:
    ...
    def find_all_by(self, actor: Actor, timeout: float = Timeouts.APPIUM_WAIT) -> list[WebElement]:
    ...
    def _set_locator_from_fallback(self, platform: str):
        for fallback_strategy, fallback_locator, fallback_platform in self.fallback_locators:
            if fallback_platform == platform:
                self.locator_strategy = fallback_strategy
                self.locator = fallback_locator
                break

    def _try_fallback_locators(self, driver, platform: str, timeout: float, multiple: bool) -> Union[WebElement, list[WebElement]]:
